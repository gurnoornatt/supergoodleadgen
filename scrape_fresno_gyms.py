#!/usr/bin/env python3
"""
Scrape real independent gyms in Fresno, CA
...
def scrape_fresno_gyms():
    """Scrape gyms in Fresno, filtering for independent gyms"""
    
    print("\n" + "="*80)
    print("SEARCHING FOR INDEPENDENT GYMS IN FRESNO, CA")
    print("="*80)
    ...
    search_queries = [
        "gym fitness center Fresno CA",
        "crossfit box Fresno CA", 
        "martial arts dojo Fresno CA",
        "boxing club Fresno CA",
        "personal training studio Fresno CA",
        "powerlifting gym Fresno CA",
        "yoga studio Fresno CA",
        "strength training gym Fresno CA"
    ]
    ...
    print(f"\n\n{'='*80}")
    print(f"FOUND {len(independent_gyms)} INDEPENDENT GYMS IN FRESNO")
    print(f"{'='*80}\n")
    ...
    output_file = f"fresno_independent_gyms_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    df.to_csv(output_file, index=False)
    print(f"💾 Results saved to: {output_file}")
    
    # Save to JSON for processing
    json_file = f"fresno_gyms_raw_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
