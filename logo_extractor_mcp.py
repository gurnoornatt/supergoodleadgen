"""
Logo extraction module with MCP Firecrawl integration for Pain-Gap Audit Automation
"""
import os
import re
import requests
...
class LogoExtractorMCP:
    """Extract business logos from websites using MCP tools with fallback generation"""
    
    def __init__(self, mcp_client=None):
        """
        Initialize with MCP client for Firecrawl access
        
        Args:
            mcp_client: MCP client instance that can call firecrawl tools
        """
    ...
    def extract_logo_from_website_html(self, html_content: str, base_url: str, business_name: str) -> Dict[str, Any]:
        """
        Extract logo from HTML content (designed to work with Firecrawl HTML)
        ...
        try:
            logger.info(f"Extracting logo for {business_name} from HTML content")
            
            # Parse HTML content
            soup = BeautifulSoup(html_content, 'html.parser')
            
            # Find logo using common selectors
            logo_url = self._find_logo_in_html(soup, base_url)
            ...
            if logo_url:
                ...
                downloaded_path = self._download_logo(logo_url, business_name)
                if downloaded_path:
                    result['logo_local_path'] = downloaded_path
                    result['success'] = True
                    logger.info(f"Successfully extracted logo for {business_name} from HTML")
                    return result
                else:
                    result['error_message'] = 'Failed to download extracted logo'
            else:
    ...
    def extract_logo_from_website(self, url: str, business_name: str) -> Dict[str, Any]:
        """
        Extract logo from business website using fallback methods
        ...
        try:
            ...
            # Try HTML parsing with requests
            logo_info = self._extract_logo_with_requests(url)
            if logo_info['success']:
                result.update(logo_info)
                result['extraction_method'] = 'requests_html'
                ...
                if downloaded_path:
                    result['logo_local_path'] = downloaded_path
                    result['success'] = True
                    logger.info(f"Successfully extracted logo for {business_name} using requests")
                    return result
    ...
    def _extract_logo_with_requests(self, url: str) -> Dict[str, Any]:
        """Extract logo using requests (fallback method)"""
        ...
        try:
            ...
            result['error_message'] = f'Requests extraction error: {str(e)}'
        ...
        for i, lead in enumerate(leads, 1):
            ...
            if not website:
                ...
                try:
                    ...
                    lead['logo_fallback_generated'] = True
                    lead['logo_quality_score'] = 100
                    lead['logo_valid'] = True
                except Exception as e:
